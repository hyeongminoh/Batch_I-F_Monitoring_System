...
trainer와 detector가 `freq_utils.py` 를 공통으로 import해 사용하며, 실행 시작 시 `ICS_WRKDAY_MST`에서 로드한 영업일 set(`biz_days`)을 함께 전달합니다.

**BUSINESS_DAY 판별 (달력 간격 계산보다 우선)**  
① 모든 도착일이 영업일 집합에 포함  
② 영업일 순서 기준 연속 간격이 모두 1  
→ 두 조건을 모두 만족하면 `BUSINESS_DAY` 반환 (공휴일·주말 건너뜀 자동 반영)

> 영업일 파일은 금→월 달력 간격이 3일이라 기존 로직에서 `IRREGULAR`로 오분류됩니다.  
> `biz_days` 기반 우선 판별로 이 문제를 해결합니다.
...
- `DAILY` / `WEEKLY` / `BUSINESS_DAY` / `IRREGULAR`는 `None` 반환 (적용 불필요)
